...
function deepEqual(a, b, doneObjectComparisons = [], key = "") {
	...
	if (a instanceof Set || b instanceof Set) {
		if (!(b instanceof Set) || !(a instanceof Set))
			return done("One value is a set but not the other")

		nextFirstSetValue: for (const aValue of a) {
		...
		nextSecondSetValue: for (const bValue of b) {
		...
		if (!(a instanceof Map) || !(b instanceof Map))
			return done("One value is a map but not the other")

		nextFirstMapValue: for (const [keyA, valueA] of a.entries()) {
		...
		nextSecondMapValue: for (const keyB of b.keys()) {
	...
	for (const key in a) {
		...
		if (!deepEqual(a[key], b[key], doneObjectComparisons, key)) return false
	}
	for (const key in b) {
		if (!(key in a) && b[key] !== undefined)
			return done(`Key '${key}' is missing in the second object`)
	}
